...
theorem Manhattan.Frozen.Operator.resolventOperator_bijective : ∀ {E : Type u_1} [_inst : NormedAddCommGroup E]
  [_inst_1 : InnerProductSpace ℂ E] [CompleteSpace E] (G : E →L[ℂ] E) {lambda : ℝ},
  0 < lambda →
    (∀ (t : ℝ), 0 ≤ t → ‖Manhattan.Operator.operatorSemigroup G t‖ ≤ 1) →
      Function.Bijective ⇑(Manhattan.Operator.resolventOperator G lambda)
-- FROZEN-STATEMENT-END
:= @Manhattan.Operator.resolventOperator_bijective
