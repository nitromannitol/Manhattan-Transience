...
One point affects what the formalization asserts.  The two displayed numerical
constants are proved with larger explicit values.  The formalized
Lemma~\ref{lem:effective-energy} carries a constant certified below $670$ in
place of $16$, and the constant of Proposition~\ref{prop:frequency} is
correspondingly larger than $2048$.  Every statement is formalized as
displayed, and the constant of Proposition~\ref{prop:frequency} is existential
there, so no formalized statement claims a numerical value it does not prove.
The loss is in the passage from Lemma~\ref{lem:parity} to
\eqref{eq:energy-reduction}, where the formalization splits the residual by
Cauchy--Schwarz and bounds the degree-three form by a majorant instead of
minimizing it exactly.
...
weighted square integral, with coefficient $16$ independent of $\lambda$,
$p$, and $\varphi$.
\end{lemma}
...
Substitution into \eqref{eq:effective-energy} leaves one scalar to
minimize.  Completion of the square gives
